...
网页与文本采集已有确定性脚本；Web 工作台支持知识概览、搜索、详情、增删改查，并可通过粘贴文本、抓取网页或上传 TXT/Markdown 导入内容，再由 organizer 自动生成标题、摘要、分类和标签草稿。Web 智能问答支持直接问答和 Agent 调度双模式：前者从 SQLite 检索证据后直接调用 qa Agent，后者先由 steward 返回严格校验的路由决策，再调用 qa Agent，并展示后端生成的调度轨迹。知识图谱提供“分类气泡”和“关系网络”双模式，可按学科、知识、主题逐层展开；标签 Jaccard 推荐也已接入 Web。RSS 定时任务和图谱持久化仍在迭代。
...
- Agent 调度模式必须先获得 `clawnote-steward` 的严格路由结果才会显示轨迹；已验证路由在内存中复用 15 分钟，并发首次请求合并为一次调用，缓存随后端重启清空。
