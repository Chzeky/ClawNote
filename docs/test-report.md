...
最新回归时间：2026-07-19。接手后重新执行 Python、Jest、ESLint 和 Vite build，全部通过。
...
| TC-42 | AI/AI技术及前后端开发等分类别名在图谱展示层正确归一化 | 通过 |
| TC-43 | 推荐接口生成按阶段排序的学习路径 | 通过 |
| TC-44 | 知识缺口建议排除数据库已有标签 | 通过 |
...
| IT-16 | 分类气泡按学科、知识、主题三级展开并可打开详情 | 通过 | PC 浏览器 D3 气泡交互验收 |
...
- Web 前端已支持工作台概览、知识浏览与 CRUD、带引用的 RAG 问答、分类气泡/关系网络双模式图谱、标签相关推荐，以及文本、网页、文件三种 organizer 智能导入方式；完整五 Agent 流水线调度仍以飞书入口为主。
